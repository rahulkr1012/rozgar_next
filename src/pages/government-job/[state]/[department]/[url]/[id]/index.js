import { GovtDeptbyState, getGovermentDeptJob, getGovermentStateURL } from '@/action/dashboard';
...
const GovernmentDeptJobs = dynamic( () => import('components/GovernmentDeptJobs/index'), { ssr: false ,});
...
getDeptStatebyURL=()=>{
  ...
  getGovermentStateURL(url,category).then(res => {
    ...
    deptJobList = () => {
        ...
        this.setState({showLoader:true})
        getGovermentDeptJob(url,id,this.state.page).then(res => {
  ...
  render() {
    const {stateJobList,deptList,joblist,showLoader,Central,stateURLList}=this.state
