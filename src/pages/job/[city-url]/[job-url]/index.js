...
const index = (props) => {
  ...
  const [detail1, setDetail1] = React.useState(getCookie(constant.keys.cd) ? JSON.parse(getCookie(constant.keys.cd)) : '')
...
export async function getServerSideProps(context) {
  ...
  const jobDetailRes = await jobDetail({
    URL: context.params['job-url']
  })
