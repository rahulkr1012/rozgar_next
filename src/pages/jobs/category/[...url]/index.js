...
class index extends Component  {
  ...
  componentDidMount() {
    getBannerList().then((res) => {
      this.setState({BannerSkill: res.result.list.map((item)=>{
       if(item.PAGE_LOCATION=="Job_by_category"){
          return item
       }
      })});
    }).catch((err) => {
  ...
  render() {
   ...
   const description=  ` Apply to ${capitalizeWords(
        this.props.router.query?.url[0]?.split("-")
      ).join(" ")} jobs on Rozgar.com. Search ${capitalizeWords(
        this.props.router.query?.url[0]?.split("-")
      ).join(" ")} vacancies. Explore openings for ${capitalizeWords(
        this.props.router.query?.url[0]?.split("-")
      ).join(" ")} across top companies now!`
