...
class index extends Component  {
  ...
  componentDidMount() {
    getBannerList().then((res) => {
      this.setState({BannerSkill: res.result.list.map((item)=>{
       if(item.PAGE_LOCATION=="Job_by_designation"){
          return item
       }
      })});
    }).catch((err) => {
  ...
  render() {
    ...
    const title = includeJobsInTitle
      ? ` ${capitalizeWords(
          this.props.router.query?.url[0]?.split("-")
        ).join(" ")}Jobs -Jobs For ${capitalizeWords(
          this.props.router.query?.url[0]?.split("-")
        ).join(" ")} - Rozgar.com`
      : `${capitalizeWords(this.props.router.query?.url[0]?.split("-")).join(
          " "
        )} Jobs - Jobs For ${capitalizeWords(this.props.router.query?.url[0]?.split("-")).join(
          " "
        )} - Rozgar.com`;
        const description=`Apply To ${capitalizeWords(this.props.router.query?.url[0]?.split("-")).join(
          " "
        )} Jobs & Vacancies On Rozgar.com. Explore ${capitalizeWords(this.props.router.query?.url[0]?.split("-")).join(
          " "
        )} Jobs In Your Desired Locations Now!`
    return (
