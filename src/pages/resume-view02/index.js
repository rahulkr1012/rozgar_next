...
import ResumeViewThree from 'components/ResumeViewThree'
...
import { getEduQualificationTypeListsForJobs } from '@/action/CandidateAction';
 ...
 class resumeViewThree extends Component {
    ...
    if (CANDIDATE_ID) {
      getResumeData(CANDIDATE_ID).then((res) => {
        this.setState({ ...this.state, cd_list: res.result },  async () => {
          ...
          // let degree_response = await Promise.all(this.state.candidateList.Education.map(async (ele) => {
          //   return this.getCourse(ele.DEGREE)
          // })
          // )


          // let course_specilization = await Promise.all(this.state.candidateList.Education.map(async (ele) => {
          //   return this.getSpecializeCourse(ele.FIELD_OF_STUDY)
          // })
          // )

          // let degree_education =  degree_response.map((ele ,index)=>{
          //       return ele.filter(innerEle=>{
          //          return  this.state.candidateList.Education[index].FIELD_OF_STUDY == innerEle.EDUCATION_QUALIFICATION_ID
          //       }) 
          //     })

          // let course_specilization_list =  course_specilization.map((ele ,index)=>{
          //       return ele.filter(innerEle=>{
          //          if((this.state.candidateList.Education[index].FIELD_OF_STUDY == innerEle.EDUCATION_QUALIFICATION_ID) && this.state.candidateList.Education[index].COURSE_SPECIALIZATION == innerEle.SPECIALIZATION_ID   )
          //             return {...this.state.candidateList.Education[index] }
          //       }) 
          //     })
          ...
          //     let f_degree_education = degree_education.flat()
          //     let f_course_specilisation = course_specilization_list.flat()

          // console.log("f_degree_education" , f_degree_education );
          // console.log("course_specilization" , f_course_specilisation );

          // let final_education = this.state.candidateList.Education.map((ele,index)=>{
          //    return {
          //     ...ele, ...f_degree_education[index] ,... f_course_specilisation[index]
          //    }
          // })  
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(resumeViewThree))
