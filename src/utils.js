...
export function generateTitle(url) {
  ...
  if (isInclude) {
    let title = url.split('?')[0].replace(/Years.*$/i, "")
    return title + "Years"
  } else return url
}
...
export const getDatePartsSecond = (input) => {
  ...
  return {
    ...
    sec: mDate.format("ss"),
    amOrpm: mDate.format("A"),
    relativeTime: mDate.fromNow().replace('min', "min").replace("seconds", "sec").replace('minute', 'min').replace('hours', 'h').replace('seconds', 'sec').replace('months', 'M')
  };
};
